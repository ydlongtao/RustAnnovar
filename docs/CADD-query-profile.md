...
A second strict run of the same 271,738-allele chromosome 1 input used one thread and a 4,096-record batch instead of 16 threads and a 10,000-record batch. It exited with code 0, reported all 271,738 alleles as `scored`, and its entire TSV output matched the first run byte for byte (`cmp`). This checks thread/batch consistency on this real-data workload. It took 213.80 seconds with peak RSS 55,428 KiB. Since both thread count and batch size changed, the timing difference does not isolate thread scaling. The partial-database and single-run limitations above still apply. The job state is retained under `cadd-giab-chr1-prefix-5499531-one-thread`.
