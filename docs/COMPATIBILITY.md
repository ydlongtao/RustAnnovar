...
公共 GIAB HG002 v4.2.1 分层抽样对照表明：在相同 AVinput、基因模型及转录本 FASTA 下，hg19/hg38 五个基因注释字段的逐等位基因一致率（仅忽略列表顺序）分别为 98.311% 和 98.346%；两套数据仍分别有 1,020 和 1,004 条实质差异。独立 VCF 转换的 Indel 表示也不完全一致。详见[英文准确度报告](ACCURACY_GIAB_2026-09-19.md)。这是与原版结果的一致性评估，不是生物学准确率。
...
- ANNOVAR 对复杂多外显子 Indel、HGVS 边界和异常 ORF 的全部历史细节；公共真实数据基线发现少量 SNV 编码后果差异，尚未实现完整一致。
